...
class PlayState extends MusicBeatState
{
	...
	function set_playbackRate(value:Float):Float
	{
		...
		Conductor.ogSFO = (ClientPrefs.safeFrames / 60) * 1000 * value;
	...
	private function generateSong(dataPath:String):Void
	{
		...
		var BPM = SONG.bpm;
		var stepLength:Float = (60 / BPM) * 1000 / 4;
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (!Std.isOfType(songNotes[3], String))
					swagNote.noteType = funkin.states.editors.ChartingState.noteTypeList[songNotes[3]]; // Backward compatibility + compatibility with Week 7 charts
				swagNote.scrollFactor.set();
				var susLength:Float = swagNote.sustainLength;

				susLength = susLength / stepLength;
				...
				if (susLength > 0)
				{
					...
					for (susNote in 0...roundSus)
					{
						...
						sustainNote.sustainLength = stepLength;
		...
		if (generatedMusic && !inCutscene)
		{
			...
			if (startedCountdown)
			{
				...
				notes.forEachAlive(function(daNote:Note)
				{
					...
					daNote.speed = (songSpeed * daNote.multSpeed) * 0.45;

					if (strumScroll) // Downscroll
						// daNote.y = (strumY + 0.45 * (Conductor.songPosition - daNote.strumTime) * songSpeed);
						daNote.distance = ((Conductor.songPosition - daNote.strumTime) * daNote.speed);
					else // Upscroll
						// daNote.y = (strumY - 0.45 * (Conductor.songPosition - daNote.strumTime) * songSpeed);
						daNote.distance = ((daNote.strumTime - Conductor.songPosition) * daNote.speed);
					...
					if (daNote.copyY)
					{
						daNote.y = strumY + daNote.pixYOffset + Math.sin(angleDir) * daNote.distance;
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		var time:Float = Conductor.stepCrochet * 1.25 / 1000;
